...
import words from '@/views/speedTest/words/russian/russian-words.json'
...
export default {
    ...
    mounted() {
        if(!this.data_text){
            this.initTextBase(60);
        }
        this.setEvents()
    },
    methods: {
        initTextBase(num) {
            let out = Array.from({length:num},(_,i) =>words[this.randomIntFromInterval(0,words.length)]).join(' ');
            this.data_text = out;
            return out;
        },
