    ...
    <section class="speed-test-window animate__animated animate__slideInUp">
        <header class="controller-header-components">
            ...
            <aside class="controller-right-window-content">
                <button class="controller-close-button" @click="$event => this.closeWindowEmitter()"><i
            ...
            <section class="controller-results-section">
                <fieldset class="controller-results">
                    ...
                    <div class="controller-statistic-text">
                        <span class="result-content">{{ String(Math.floor(this.TimeValue / 60) % 60).padStart(2, "0") }}:{{
                            String(Math.floor(this.TimeValue) % 60).padStart(2, "0") }}</span>
                    </div>
                </fieldset>
                <fieldset class="controller-results">
...
export default {
    ...
    watch: {
        'data_SelectValue': {
            immediate: true,
            handler(val) {
                if (val >= this.fib(this.data_chart_size + 1)) {
                    this.data_chart_size++;
                    this.myChart.data.labels = this.getLabels();
                    console.log(this.myChart.data.labels);
                    console.log(this.data_chart_size)
                    this.myChart.update();
                }
            },
            deep: true
    ...
    methods: {
        ...
        closeWindowEmitter(event) {
            this.$emit('exit', { status: true, element: this.$el });
        },
        getSpeed() {
        ...
        getAccuracy() {
            // let out = ((this.data_SelectValue+this.MistakesValues.length) / this.data_SelectValue * 100).toFixed(1);
        ...
        getLabels() {
            return Array.from({
                length: this.SelectValue > this.data_chart_size ?
...
.speed-test-window {
    ...
    .main-window-content {
        ...
        display: flex;
        flex-direction: column;
        padding: calc(var(--standard-border-radius) + var(--padding-boost));
    }
